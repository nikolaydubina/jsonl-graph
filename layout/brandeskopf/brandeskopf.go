...
func verticalAlignment(g layout.LayeredGraph, typeOneSegments map[[2]uint64]bool) (root map[uint64]uint64, align map[uint64]uint64) {
...
// part of Alg 3.
func placeBlock(g layout.LayeredGraph, x map[uint64]int, root map[uint64]uint64, align map[uint64]uint64, sink map[uint64]uint64, shift map[uint64]int, delta int, v uint64) {
	if _, ok := x[v]; !ok {
		x[v] = 0
		flag := true
...
func horizontalCompaction(g layout.LayeredGraph, root map[uint64]uint64, align map[uint64]uint64, delta int) (x map[uint64]int) {
